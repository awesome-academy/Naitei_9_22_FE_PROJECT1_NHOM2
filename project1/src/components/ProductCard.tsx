    ...
    <div className={`relative border rounded-lg shadow hover:shadow-xl overflow-hidden bg-white group transition-all duration-300 h-full flex flex-col ${className}`}>
      <div className="relative">
        <Image
          ...
          width={400}
          height={300}
          className="w-full aspect-square sm:aspect-[4/3] lg:aspect-[4/3] object-cover group-hover:brightness-75 transition-all duration-300"
        />
        {hasDiscount && (
        ...
        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 flex items-center justify-center gap-1 sm:gap-2 transition duration-300">
          <button className="bg-green-500 text-white px-2 py-1 sm:px-3 sm:py-1 text-xs sm:text-sm rounded hover:bg-green-600 transition-colors">
            <span className="hidden sm:inline">MUA NGAY</span>
            <span className="sm:hidden">MUA</span>
          </button>
          <button className="bg-white border border-gray-300 p-1.5 sm:p-2 rounded hover:bg-gray-50 transition-colors">
      ...
      <div className="p-2 sm:p-3 lg:p-4 text-center flex-1 flex flex-col justify-between">
        <div>
          <h3 className="text-sm sm:text-base lg:text-lg font-medium mb-1 line-clamp-2">{name}</h3>
          <div className="text-yellow-500 text-xs sm:text-sm mb-1">★★★★★</div>
        </div>
